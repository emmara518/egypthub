...
import type { ExperienceCarouselProps } from './ExperienceCarousel.types';

export function ExperienceCarousel({ experiences, className }: ExperienceCarouselProps) {
  const [current, setCurrent] = useState(0);
  const reducedMotion = useReducedMotion();
  const itemsPerView = 3;
  const max = Math.max(0, experiences.length - itemsPerView);
  ...
  return (
    <div className={cn('relative', className)}>
      <div className="overflow-hidden">
        ...
        >
          {experiences.map((exp) => (
            <div key={exp.id} className="min-w-[calc(33.333%-16px)] flex-shrink-0">
              <ExperienceCard {...exp} />
            </div>
          ))}
        </motion.div>
