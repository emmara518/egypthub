      ...
      <div className="py-16 relative overflow-hidden">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-8 items-start">
              ...
              >
                ...
                <div className="relative h-[380px]">
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-10">
                    <span className="text-[10px] font-bold text-theme-gold font-english tracking-[0.2em] mb-3 block">CURATED LUXURY</span>
                    <h2 className="text-3xl md:text-4xl font-bold font-display mb-3 leading-[1.1]">
                    ...
                    <motion.a
                      href="#"
                      whileHover={{ x: 4 }}
                      className="inline-flex items-center gap-2 text-sm font-semibold text-theme-gold font-english group/btn w-fit"
                    >
                      View Collection
                ...
                {/* Partner logos row */}
                <div className="bg-[#0a0e1a]/90 backdrop-blur-sm border-t border-white/[0.04] px-8 py-5">
                  <div className="flex items-center justify-between">
                    ...
                    <motion.button
                      whileHover={{ x: 4 }}
                      className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center hover:border-theme-gold/30 transition-all shrink-0 ml-4"
                    >
            ...
            {/* Right: Mobile Phone Overlay */}
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: '-100px' }}
              className="hidden lg:block shrink-0 relative -mt-20"
            >
              ...
              <div className="w-[260px] h-[540px] rounded-[40px] bg-gradient-to-b from-[#1A2235] to-[#0F1525] p-[3px] shadow-[0_20px_80px_rgba(0,0,0,0.6),0_0_40px_rgba(212,162,76,0.05)]">
                <div className="w-full h-full rounded-[37px] bg-[#080C18] overflow-hidden relative">
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-6 bg-[#080C18] rounded-b-2xl z-20" />

                  <div className="h-full bg-gradient-to-b from-[#080C18] to-[#0a1020] p-4 pt-8 overflow-y-auto">
                    ...
                    <div className="mb-4">
                      <p className="text-[9px] text-white/50 font-english">Hello, Ahmed 👋</p>
                      <p className="text-[13px] font-bold mt-1 leading-snug">Where do you want to explore today?</p>
                    </div>

                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {['Relaxation', 'Adventure', 'Culture', 'Food', 'Luxury'].map((tag) => (
                        <span key={tag} className="px-2 py-0.5 rounded-full bg-white/[0.04] text-[7px] text-white/50 font-english border border-white/[0.04]">
                          {tag}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="mb-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[9px] font-bold">Popular Destinations</span>
                            ...
                            <div className="p-1.5">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-0.5">
                                  ...
                                  <span className="text-[6px] font-english text-theme-gold">{dest.rating}</span>
                                </div>
                              </div>
                            </div>
                          </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[9px] font-bold">Top Experiences</span>
                        <span className="text-[7px] text-theme-gold font-english">See All</span>
                      </div>
                      {topExperiences.map((exp) => (
                        <div key={exp.name} className="flex gap-3 p-2 rounded-xl bg-[#0F1525] border border-white/[0.04]">
                          <div className="w-11 h-11 rounded-lg overflow-hidden shrink-0">
                            <img src={exp.img} alt={exp.name} className="w-full h-full object-cover" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-[8px] font-bold truncate">{exp.name}</p>
                            <p className="text-[7px] text-theme-gold font-english">{exp.price}</p>
                            <div className="flex items-center gap-0.5 mt-0.5">
                              <HiStar className="w-2 h-2 text-theme-gold fill-theme-gold" />
                              <span className="text-[6px] font-english">{exp.rating}</span>
                              <span className="text-[6px] text-white/30 font-english">({exp.reviews})</span>
                            </div>
                          </div>
                        </div>
                      ))}
