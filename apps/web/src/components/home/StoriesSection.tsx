...
export default function StoriesSection() {
  ...
  return (
    <section className="py-24 bg-[#080C18] relative">
      <div className="absolute top-0 left-1/4 w-64 h-64 opacity-[0.03] pointer-events-none">
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-10 items-start">
          ...
          >
            <span className="text-[10px] font-bold text-theme-gold font-english tracking-[0.2em] mb-4 block">STORIES</span>
            <h2 className="text-4xl md:text-5xl font-bold font-display leading-[1.1] mb-4">
            ...
            <motion.a
              href="#"
              whileHover={{ x: 4 }}
              className="inline-flex items-center gap-2 text-sm font-medium text-theme-gold font-english group"
            >
              Explore All Stories
          ...
          <div className="relative">
            {/* Carousel arrows */}
            <div className="absolute -left-5 top-1/2 -translate-y-1/2 z-10 hidden lg:block">
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                onClick={() => scroll('left')}
                className="w-10 h-10 rounded-full bg-[#0F1525] border border-white/[0.08] flex items-center justify-center hover:border-theme-gold/30 transition-all shadow-[0_4px_12px_rgba(0,0,0,0.3)]"
              >
                <HiChevronLeft className="w-5 h-5 text-white/60" />
              </motion.button>
            </div>
            <div className="absolute -right-5 top-1/2 -translate-y-1/2 z-10 hidden lg:block">
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
            ...
            <div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-6 scrollbar-hide">
              {stories.map((story, i) => (
                <motion.div
                  key={story.title}
                  ...
                  className="min-w-[250px] max-w-[250px] rounded-xl overflow-hidden bg-[#0F1525] border border-white/[0.06] group cursor-pointer hover:border-theme-gold/25 transition-all duration-500 shadow-[0_4px_24px_rgba(0,0,0,0.3)] hover:shadow-[0_8px_40px_rgba(0,0,0,0.5),0_0_30px_rgba(212,162,76,0.05)]"
                >
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <span className={`absolute top-3 left-3 px-3 py-1 rounded-full text-[9px] font-bold font-english tracking-wider backdrop-blur-sm border ${tagColors[story.tag]}`}>
                      {story.tag}
                    </span>
                    <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="w-10 h-10 rounded-full bg-theme-gold/90 backdrop-blur-sm flex items-center justify-center shadow-[0_0_20px_rgba(212,162,76,0.3)]">
                        <HiPlay className="w-4 h-4 text-[#080C18] ml-0.5" />
                      </div>
                    </div>
                  </div>
                  <div className="p-4">
                    <h4 className="font-bold text-sm mb-1.5 group-hover:text-theme-gold transition-colors duration-300">{story.title}</h4>
                    <p className="text-[11px] text-white/50 mb-3 leading-relaxed line-clamp-2">{story.desc}</p>
