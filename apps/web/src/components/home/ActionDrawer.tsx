...
function PriceAlertModal({ onClose }: { onClose: () => void }) {
  ...
  return done ? (
    <div className="p-8 text-center"><div className="w-14 h-14 rounded-full bg-green-500/20 flex items-center justify-center mx-auto mb-3"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--success)" strokeWidth="2"><path d="M20 6L9 17l-5-5"/></svg></div><p className="text-green-400 font-bold font-english">Alert Set!</p><p className="text-xs text-white/40 mt-1">We&apos;ll email you</p></div>
  ) : (
...
interface Action { icon: React.ComponentType<{ size?: number }>; label: string; color: string; rgb: string; modal: React.ReactNode; }
...
export default function ActionDrawer() {
  ...
  const actions: Action[] = [
    { icon: Compass, label: 'بناء رحلتك', color: 'var(--gold)', rgb: 'var(--gold-rgb)', modal: <div className="p-4 space-y-2">
    ...
    { icon: Search, label: 'بحث', color: 'var(--gold)', rgb: 'var(--gold-rgb)', modal: <SearchModal onClose={() => setActiveModal(null)} /> },
    { icon: Location, label: 'استكشف', color: 'var(--cyan)', rgb: '65, 190, 220', modal: <div className="p-4 space-y-2">
    ...
    { icon: Star, label: 'المساعد الذكي', color: 'var(--purple)', rgb: '139, 92, 246', modal: <div className="p-4 space-y-2">
    ...
    { icon: Play, label: 'جولة 360', color: 'var(--pink)', rgb: '236, 72, 153', modal: <div className="p-4 text-center"><div className="aspect-video rounded-xl bg-gradient-to-br from-theme-gold/10 to-theme-bg flex items-center justify-center border border-theme-gold/20 mb-3"><Play size={32} /></div><p className="text-sm text-white/60 font-english">Drag to explore panoramic views</p></div> },
    { icon: CalIcon, label: 'التقويم', color: 'var(--cyan)', rgb: '65, 190, 220', modal: <CalendarModal onClose={() => setActiveModal(null)} /> },
    { icon: Dollar, label: 'العملة', color: 'var(--success)', rgb: '16, 185, 129', modal: <CurrencyModal onClose={() => setActiveModal(null)} /> },
    { icon: Sun, label: 'الطقس', color: 'var(--warning)', rgb: '245, 158, 11', modal: <div className="p-4 text-center text-white/40 font-english text-sm">Weather: Cairo 34° ☀️<br />Luxor 42° 🔥<br />Alex 29° 🌊</div> },
    { icon: Globe, label: 'اللغة', color: 'var(--purple)', rgb: '139, 92, 246', modal: <div className="p-6 space-y-2">
            ...
            <div className="grid grid-cols-5 gap-3 p-5 max-h-[50vh] overflow-y-auto scrollbar-hide">
              {actions.map((a) => (
                <button key={a.label} onClick={() => setActiveModal(a.label)}
                  ...
                  aria-label={a.label}
                >
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center transition-all group-hover:scale-110" style={{ backgroundColor: `rgba(${a.rgb}, 0.15)` }}>
                    <a.icon size={18} />
                  </div>
