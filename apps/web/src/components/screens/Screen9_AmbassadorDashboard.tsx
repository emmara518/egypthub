      ...
      <div className="flex">
        ...
        <motion.aside initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }}
          ...
          <div className="mb-6">
            ...
            <p className="text-[10px] text-theme-muted font-english">Ambassador Dashboard</p>
          </div>
          ...
          <div className="flex items-center gap-3 p-3 rounded-xl bg-theme-card border border-theme-border mb-4">
            <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-theme-gold to-accent-orange flex items-center justify-center">
              <span className="text-dark-900 font-bold text-sm">س</span>
            </div>
            <div>
              <p className="text-xs font-bold font-english">EGYPTHUB</p>
              <p className="text-[9px] text-theme-muted">سفير نشط</p>
            </div>
          </div>
          ...
          <div className="mt-6 p-3 rounded-xl bg-theme-gold/5 border border-theme-gold/15">
            ...
            <p className="text-[9px] text-theme-muted">إدارة الشبكة والإحالات</p>
          </div>
        </motion.aside>
        ...
        <div className="flex-1 min-w-0 p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
              className="xl:col-span-2 bg-theme-card rounded-2xl border border-theme-border p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                {performanceMetrics.map((m, i) => (
                  <div key={i} className="bg-theme-surface rounded-xl p-3 border border-theme-border">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] text-theme-muted">{m.label}</span>
                    </div>
                ...
                {networkStats.map((s, i) => (
                  <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-theme-surface border border-theme-border">
                    <div>
                      ...
                      <p className="text-[9px] text-theme-muted">{s.change}</p>
                    </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} className="bg-theme-surface rounded-xl p-3 border border-theme-border text-center">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              ...
              <div className="text-center">
                ...
                <p className="text-[10px] text-theme-muted mb-3">سفير نشط منذ 2024</p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
              ...
              <div className="pt-4 border-t border-theme-border">
                <p className="text-[10px] text-theme-muted mb-2">اتجاه الأداء الشهري</p>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex gap-0.5">
                        {Array.from({ length: 5 }).map((_, s) => (
                          <HiStar key={s} className={`text-xs ${s < t.stars ? 'text-theme-gold' : 'text-theme-elevated'}`} />
                        ))}
                      </div>
                    </div>
                    <p className="text-[10px] text-theme-secondary">{t.text}</p>
                    <p className="text-[9px] text-theme-muted mt-1">{t.date}</p>
                  </div>
                ))}
              </div>
            </motion.div>
              ...
              <div className="space-y-2">
                {referrals.map((r, i) => (
                  <div key={i} className="flex items-center gap-3 p-2.5 rounded-xl bg-theme-surface border border-theme-border">
                    <div className="w-9 h-9 rounded-full bg-theme-elevated flex items-center justify-center overflow-hidden shrink-0 relative">
                      <Image src={r.img} alt={r.name} fill className="object-cover" />
                    </div>
                    <div className="flex-1">
                      <p className="text-xs font-bold">{r.name}</p>
                      <p className="text-[9px] text-theme-muted">{r.status}</p>
                    </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.65 }}
              ...
              <div className="bg-theme-surface rounded-xl p-4 border border-theme-border">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-theme-secondary">نسبة الإنجاز الشهرية</span>
