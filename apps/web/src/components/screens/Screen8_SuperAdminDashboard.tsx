    ...
    <div className="min-h-screen bg-theme-bg text-white font-arabic">
      <div className="flex h-screen overflow-hidden">
        ...
        <motion.aside initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }}
          className="w-64 shrink-0 bg-theme-surface border-l border-theme-border p-4 flex flex-col h-full overflow-y-auto">
            ...
            {sidebarItems.map((item, i) => (
              <button key={i} onClick={() => setActiveSidebar(i)}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${
                  activeSidebar === i ? 'bg-theme-gold/10 text-theme-gold border border-theme-gold/20' : 'text-theme-secondary hover:text-white hover:bg-theme-elevated'
                }`}>
          ...
          <div className="mt-6 p-4 rounded-2xl bg-theme-card border border-theme-border">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
              <p className="text-[10px] text-theme-secondary">حالة النظام</p>
            </div>
            <p className="text-sm font-bold text-green-400 font-english mb-1">99.98% Uptime</p>
            <p className="text-[9px] text-theme-muted">جميع الأنظمة تعمل بكفاءة</p>
          </div>
        </motion.aside>
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full">
          {/* Header */}
          <header className="h-16 border-b border-theme-border bg-theme-surface flex items-center justify-between px-6 shrink-0">
            <div>
              <h2 className="text-sm font-bold">النظرة العامة العالمية</h2>
              <p className="text-[10px] text-theme-secondary">آخر تحديث: منذ دقيقتين</p>
            </div>
            <div className="flex items-center gap-4">
              <div className="relative">
                <HiSearch className="absolute right-3 top-1/2 -translate-y-1/2 text-theme-muted" />
                <input placeholder="بحث في النظام..." className="bg-theme-card border border-theme-border rounded-lg pr-9 pl-4 py-1.5 text-xs w-64 outline-none focus:border-theme-gold/40" />
              </div>
              <button className="relative w-8 h-8 rounded-full bg-theme-card border border-theme-border flex items-center justify-center text-theme-secondary hover:text-theme-gold transition-colors">
                <HiBell />
          ...
          <div className="flex-1 overflow-y-auto p-6">
              ...
              {globalKpis.map((kpi, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-10 h-10 rounded-xl bg-theme-elevated flex items-center justify-center">
                  ...
                  <p className="text-xs text-theme-secondary">{kpi.label}</p>
                </motion.div>
              ))}
            </div>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                className="xl:col-span-2 bg-theme-card rounded-2xl border border-theme-border p-5 flex flex-col">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h3 className="font-bold text-sm">التدفق المالي والأرباح</h3>
                    <p className="text-[10px] text-theme-muted">مقارنة الإيرادات بالمصروفات للعام الحالي</p>
                  </div>
                  <button className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-theme-elevated text-xs text-theme-secondary">
                    <span>2024</span>
                    <HiAdjustments className="text-sm" />
                  </button>
                </div>
                {/* Simulated Complex Chart */}
                <div className="flex-1 relative min-h-[250px] border-b border-l border-theme-border pb-2 pl-2">
                  <div className="absolute inset-0 flex flex-col justify-between pt-2">
                    {[100, 75, 50, 25, 0].map(v => (
                      <div key={v} className="border-t border-theme-border/50 w-full relative">
                        <span className="absolute -left-6 -top-2 text-[8px] text-theme-muted font-english">{v}M</span>
                      </div>
                    ))}
                  </div>
                  <div className="absolute inset-0 flex items-end justify-between px-4 pb-0 z-10">
                    ...
                    {[45, 60, 35, 75, 55, 90, 85, 110, 95, 120, 105, 130].map((h, i) => (
                      <div key={i} className="flex flex-col items-center gap-1 group">
                        ...
                        <span className="text-[8px] text-theme-muted font-english mt-2 group-hover:text-white transition-colors">
                ...
                <div className="flex justify-center gap-6 mt-4">
                  <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-theme-gold" /><span className="text-xs text-theme-secondary">الإيرادات</span></div>
                  <div className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-accent-orange" /><span className="text-xs text-theme-secondary">الأرباح الصافية</span></div>
                </div>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
                ...
                <p className="text-[10px] text-theme-muted mb-6">توزيع الحجوزات والإيرادات حسب المنطقة</p>
                <div className="space-y-4">
                  {regionalData.map((r, i) => (
                    <div key={i}>
                      ...
                      <div className="h-2 w-full bg-theme-elevated rounded-full overflow-hidden">
                ...
                <div className="mt-8 relative h-32 rounded-xl overflow-hidden border border-theme-border flex items-center justify-center bg-theme-surface">
                  <HiGlobe className="text-[100px] text-theme-elevated absolute" />
                  <div className="relative z-10 text-center">
                    <p className="text-2xl font-bold font-english text-theme-gold">4</p>
                    <p className="text-[10px] text-theme-secondary">مناطق نشطة رئيسية</p>
                  </div>
                </div>
              </motion.div>
            </div>
                    ...
                    <div key={i} className="p-3 rounded-xl bg-theme-surface border border-theme-border">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <HiServer className="text-theme-secondary" />
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <div className="text-center"><p className="text-[10px] text-theme-muted mb-0.5 font-english">Uptime</p><p className="text-xs font-bold font-english">{sys.uptime}</p></div>
                        <div className="text-center"><p className="text-[10px] text-theme-muted mb-0.5 font-english">Ping</p><p className="text-xs font-bold font-english">{sys.ping}</p></div>
                        <div className="text-center"><p className="text-[10px] text-theme-muted mb-0.5 font-english">Load</p><p className="text-xs font-bold font-english">{sys.load}</p></div>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {recentAuditLogs.map((log, i) => (
                    <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-theme-surface border border-theme-border">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[9px] text-theme-muted font-english">
                          <span className="flex items-center gap-1"><HiUser /> {log.user}</span>
                          <span className="flex items-center gap-1"><HiGlobe /> {log.ip}</span>
                        </div>
                      </div>
                      <span className="text-[9px] text-theme-muted shrink-0 whitespace-nowrap">{log.time}</span>
                    </div>
                  ))}
                </div>
                <button className="w-full mt-4 py-2 rounded-xl border border-theme-border text-xs text-theme-secondary hover:text-white hover:bg-theme-elevated transition-colors">
                  عرض جميع السجلات
                </button>
              </motion.div>
            </div>
