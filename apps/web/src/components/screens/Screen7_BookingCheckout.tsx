...
const PhoneFrame = ({ children, active = false }: { children: React.ReactNode; active?: boolean }) => (
  <div className={`w-[220px] rounded-[24px] overflow-hidden border-2 transition-all duration-500 shrink-0 ${
    active ? 'border-theme-gold/50 shadow-[0_0_30px_rgba(212,162,76,0.15)]' : 'border-theme-border'
  } bg-theme-bg`}>
    <div className="h-6 bg-theme-surface flex items-center justify-between px-4 relative">
      ...
      <div className="flex gap-1"><div className="w-2.5 h-1.5 rounded-sm border border-theme-muted" /></div>
    </div>
      ...
      <div className="max-w-[1500px] mx-auto px-6 py-8 flex gap-6">
        ...
        <motion.aside initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="w-64 shrink-0">
          <div className="sticky top-8 space-y-4">
            <div className="bg-theme-card rounded-2xl border border-theme-border p-5">
              ...
              <div className="flex gap-2 flex-wrap">
                {['اختيار التاريخ', 'تحديد المسافرين', 'تخصيص التجربة', 'مراجعة الحجز', 'تذاكر وبطاقات رقمية'].map(f => (
                  <span key={f} className="flex items-center gap-1 text-[9px] text-theme-secondary">
            ...
            {/* Step navigator */}
            <div className="bg-theme-card rounded-2xl border border-theme-border p-4 space-y-0.5">
              {steps.map((step, i) => (
                <button key={i} onClick={() => setActiveStep(i)}
                  className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-right transition-all ${
                    activeStep === i ? 'bg-theme-gold/10 text-theme-gold font-medium' : 'text-theme-secondary hover:text-white hover:bg-theme-elevated'
                  }`}>
                  <span className={`w-5 h-5 rounded text-[8px] font-bold flex items-center justify-center font-english ${
                    activeStep === i ? 'bg-theme-gold text-dark-900' : i < activeStep ? 'bg-green-500/20 text-green-400' : 'bg-theme-elevated text-theme-muted'
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center">
                <p className="text-[10px] text-theme-muted mb-2">01. اختيار التاريخ</p>
                <PhoneFrame active={activeStep === 0}>
                  <div className="px-3 py-2">
                    <div className="flex items-center justify-between mb-2">
                      <HiChevronRight className="text-theme-muted text-xs" />
                      <span className="text-[9px] font-bold font-english">يناير 2025</span>
                      <HiChevronLeft className="text-theme-muted text-xs" />
                    </div>
                    <div className="grid grid-cols-7 gap-0.5 mb-3">
                      {dayNames.map(d => (
                        <span key={d} className="text-[7px] text-theme-muted text-center py-1">{d}</span>
                      ))}
                      {/* offset */}
                      ...
                      {calendarDays.map(d => (
                        <button key={d} onClick={() => setSelectedDate(d)}
                          className={`text-[8px] py-1 rounded transition-all ${
                            d === selectedDate ? 'bg-theme-gold text-dark-900 font-bold' :
                            d > 13 && d < 20 ? 'text-white hover:bg-theme-elevated' : 'text-theme-muted'
                          }`}>{d}</button>
                      ))}
                    </div>
                    <div className="bg-theme-card rounded-xl p-2 border border-theme-border mb-2">
                      <p className="text-[7px] text-theme-muted">الوقت المختار</p>
                      <p className="text-[9px] font-english">09:00 صباحاً</p>
                    </div>
                    <button className="w-full py-2 rounded-xl bg-gradient-to-l from-theme-gold to-accent-orange text-dark-900 font-bold text-[9px]">التالي</button>
                  </div>
                </PhoneFrame>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="flex flex-col items-center">
                <p className="text-[10px] text-theme-muted mb-2">02. تحديد المسافرين</p>
                <PhoneFrame active={activeStep === 1}>
                  <div className="px-3 py-3">
                    ...
                    <div className="space-y-2 mb-3">
                      <div className="flex items-center justify-between p-2.5 rounded-xl bg-theme-card border border-theme-border">
                        ...
                        <div className="flex items-center gap-3">
                          <button onClick={() => setAdults(Math.max(1, adults - 1))} className="w-6 h-6 rounded-lg bg-theme-elevated text-theme-secondary text-xs flex items-center justify-center">−</button>
                      ...
                      <div className="flex items-center justify-between p-2.5 rounded-xl bg-theme-card border border-theme-border">
                        ...
                        <div className="flex items-center gap-3">
                          <button onClick={() => setChildren(Math.max(0, children_ - 1))} className="w-6 h-6 rounded-lg bg-theme-elevated text-theme-secondary text-xs flex items-center justify-center">−</button>
                    ...
                    <div className="bg-theme-gold/5 rounded-xl p-2.5 border border-theme-gold/15 mb-3">
                      <div className="flex justify-between"><span className="text-[8px] text-theme-secondary">الإجمالي</span><span className="text-sm font-bold text-theme-gold font-english">EGP 3,600</span></div>
                    </div>
                    <button className="w-full py-2 rounded-xl bg-gradient-to-l from-theme-gold to-accent-orange text-dark-900 font-bold text-[9px]">التالي</button>
                  </div>
                </PhoneFrame>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex flex-col items-center">
                <p className="text-[10px] text-theme-muted mb-2">03. تخصيص التجربة</p>
                <PhoneFrame active={activeStep === 2}>
                  <div className="px-3 py-3">
                    ...
                    {[{ label: 'تصوير احترافي', price: '+EGP 200', checked: true },
                      ...
                      <div key={i} className="flex items-center justify-between p-2.5 rounded-xl bg-theme-card border border-theme-border mb-1.5">
                        <div className="flex items-center gap-2">
                          <div className={`w-4 h-4 rounded border-2 flex items-center justify-center ${opt.checked ? 'bg-theme-gold border-theme-gold' : 'border-theme-border'}`}>
                            {opt.checked && <span className="text-dark-900 text-[7px]">✓</span>}
                          </div>
                    ...
                    <div className="bg-theme-gold/5 rounded-xl p-2.5 border border-theme-gold/15 mt-3 mb-3">
                      <div className="flex justify-between"><span className="text-[8px] text-theme-secondary">الإجمالي</span><span className="text-sm font-bold text-theme-gold font-english">EGP 4,100</span></div>
                    </div>
                    <button className="w-full py-2 rounded-xl bg-gradient-to-l from-theme-gold to-accent-orange text-dark-900 font-bold text-[9px]">التالي</button>
                  </div>
                </PhoneFrame>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="flex flex-col items-center">
                <p className="text-[10px] text-theme-muted mb-2">04. مراجعة الحجز</p>
                  ...
                  <div className="px-3 py-3">
                    ...
                    <div className="space-y-1.5">
                      {[{ label: 'التاريخ', val: '15 يناير 2025' }, { label: 'الوقت', val: '09:00 صباحاً' }, { label: 'المسافرون', val: '2 بالغين + 1 طفل' }, { label: 'الإضافات', val: 'تصوير + نقل VIP' }].map(r => (
                        <div key={r.label} className="flex justify-between p-2 rounded-lg bg-theme-card">
                          <span className="text-[7px] text-theme-muted">{r.label}</span>
                    ...
                    <button className="w-full py-2 rounded-xl bg-gradient-to-l from-theme-gold to-accent-orange text-dark-900 font-bold text-[9px]">تأكيد والدفع</button>
                  </div>
                </PhoneFrame>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex flex-col items-center">
                <p className="text-[10px] text-theme-muted mb-2">05. بيانات المسافر</p>
                  ...
                  <div className="px-3 py-3">
                    ...
                    <div className="space-y-2">
                      {['الاسم الكامل', 'البريد الإلكتروني', 'رقم الهاتف', 'الجنسية', 'رقم جواز السفر'].map(f => (
                        <div key={f}>
                          <p className="text-[7px] text-theme-muted mb-0.5">{f}</p>
                          <input className="w-full bg-theme-card rounded-lg px-2.5 py-2 text-[8px] border border-theme-border outline-none focus:border-theme-gold/30 placeholder:text-theme-muted" />
                        </div>
                      ))}
                    </div>
                    <button className="w-full mt-3 py-2 rounded-xl bg-gradient-to-l from-theme-gold to-accent-orange text-dark-900 font-bold text-[9px]">التالي</button>
                  </div>
                </PhoneFrame>
              </motion.div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-5">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="flex flex-col items-center">
                <p className="text-[10px] text-theme-muted mb-2">06. الدفع</p>
                  ...
                  <div className="px-3 py-3">
                    ...
                    <div className="space-y-1.5 mb-3">
                      {['بطاقة ائتمان', 'تحويل بنكي', 'محفظة إلكترونية'].map((m, i) => (
                        <div key={m} className={`flex items-center gap-2 p-2.5 rounded-xl border ${i === 0 ? 'bg-theme-gold/5 border-theme-gold/25' : 'bg-theme-card border-theme-border'}`}>
                          <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${i === 0 ? 'border-theme-gold' : 'border-theme-border'}`}>
                    ...
                    <input placeholder="رقم البطاقة" className="w-full bg-theme-card rounded-lg px-2.5 py-2 text-[8px] border border-theme-border outline-none mb-1.5 font-english placeholder:text-theme-muted" />
                    <div className="flex gap-1.5 mb-3">
                      <input placeholder="MM/YY" className="flex-1 bg-theme-card rounded-lg px-2.5 py-2 text-[8px] border border-theme-border outline-none font-english placeholder:text-theme-muted" />
                      <input placeholder="CVV" className="w-16 bg-theme-card rounded-lg px-2.5 py-2 text-[8px] border border-theme-border outline-none font-english placeholder:text-theme-muted" />
                    </div>
                    ...
                    <button className="w-full py-2 rounded-xl bg-gradient-to-l from-theme-gold to-accent-orange text-dark-900 font-bold text-[9px]">ادفع الآن</button>
                  </div>
                </PhoneFrame>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-col items-center">
                <p className="text-[10px] text-theme-muted mb-2">07. تأكيد</p>
                <PhoneFrame active={activeStep === 6}>
                  <div className="px-3 py-4 text-center">
                    ...
                    <p className="text-[8px] text-theme-secondary mb-3">رقم الحجز: EH-4521</p>
                    <div className="bg-theme-card rounded-xl p-3 border border-theme-border text-right mb-3">
                      <p className="text-[8px] font-bold mb-1">رحلة سفاري في الصحراء</p>
                      <p className="text-[7px] text-theme-muted">15 يناير 2025 • 09:00 صباحاً</p>
                      <p className="text-[7px] text-theme-muted">3 مسافرين</p>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button className="flex-1 py-1.5 rounded-lg bg-theme-gold text-dark-900 text-[8px] font-bold">التذكرة</button>
                    </div>
                  </div>
                </PhoneFrame>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="flex flex-col items-center">
                <p className="text-[10px] text-theme-muted mb-2">08. التذكرة الرقمية</p>
                <PhoneFrame active={activeStep === 7}>
                  <div className="px-3 py-3 text-center">
                    <div className="bg-theme-card rounded-2xl border border-theme-gold/20 p-4 mb-3">
                      <div className="flex items-center gap-2 justify-center mb-3">
                        <div className="w-6 h-6 rounded bg-gradient-to-br from-theme-gold to-accent-orange flex items-center justify-center">
                          <span className="text-[8px] text-dark-900 font-bold">م</span>
                        </div>
                        <span className="text-[8px] font-english font-bold">EGYPTHUB</span>
                      </div>
                      <p className="text-[9px] font-bold mb-1">رحلة سفاري في الصحراء</p>
                      <p className="text-[7px] text-theme-muted mb-3">15 يناير 2025 | 09:00 صباحاً</p>
                      {/* QR placeholder */}
                      <div className="w-28 h-28 mx-auto bg-white rounded-xl p-2 mb-3">
                        <div className="w-full h-full grid grid-cols-7 gap-0.5">
                          {Array.from({ length: 49 }).map((_, i) => (
                            <div key={i} className={`rounded-sm ${Math.random() > 0.5 ? 'bg-theme-bg' : 'bg-white'}`} />
                          ))}
                        </div>
                      </div>
                      <p className="text-[8px] text-theme-muted font-english">EH-4521-SAFARI</p>
                    </div>
                    <p className="text-[7px] text-theme-muted">امسح الكود عند نقطة الدخول</p>
                  </div>
                </PhoneFrame>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex flex-col items-center">
                <p className="text-[10px] text-theme-muted mb-2">09. تتبع الرحلة</p>
                      ...
                      {[{ time: '09:00', label: 'نقطة الانطلاق', status: 'done' },
                        ...
                        <div key={i} className="flex gap-3 relative">
                          <div className="flex flex-col items-center">
                            ...
                            {i < 4 && <div className={`w-0.5 flex-1 mt-1 ${s.status === 'done' ? 'bg-green-400/30' : 'bg-theme-border'}`} />}
                          </div>
                          <div className="pb-2">
                            <span className="text-[7px] text-theme-muted font-english">{s.time}</span>
                            <p className={`text-[8px] ${s.status === 'current' ? 'text-theme-gold font-bold' : 'text-theme-secondary'}`}>{s.label}</p>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }} className="flex flex-col items-center">
                <p className="text-[10px] text-theme-muted mb-2">10. التقييم</p>
                <PhoneFrame active={activeStep === 9}>
                  <div className="px-3 py-4 text-center">
                    <p className="text-[9px] font-bold mb-1">كيف كانت تجربتك؟</p>
                    <p className="text-[7px] text-theme-muted mb-4">نحب نسمع رأيك</p>
                    <div className="flex items-center justify-center gap-2 mb-4">
                      {[1, 2, 3, 4, 5].map(s => (
                        <button key={s} onClick={() => setRating(s)}>
                          <HiStar className={`text-2xl ${s <= rating ? 'text-theme-gold' : 'text-theme-elevated'}`} />
                        </button>
                      ))}
                    </div>
                    {rating > 0 && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                        <textarea placeholder="اكتب تعليقك..." rows={3}
                          className="w-full bg-theme-card rounded-xl px-3 py-2 text-[8px] border border-theme-border outline-none resize-none placeholder:text-theme-muted mb-3" />
                        <button className="w-full py-2 rounded-xl bg-gradient-to-l from-theme-gold to-accent-orange text-dark-900 font-bold text-[9px]">إرسال التقييم</button>
                      </motion.div>
                    )}
                  </div>
                </PhoneFrame>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex flex-col items-center">
                <p className="text-[10px] text-theme-muted mb-2">11. العروض والولاء</p>
                <PhoneFrame active={activeStep === 10}>
                  <div className="px-3 py-3 text-center">
                    ...
                    <p className="text-[7px] text-theme-muted mb-3">نقطة مكتسبة</p>
                    <div className="space-y-1.5">
                      {[{ offer: 'خصم 15% على الحجز القادم', pts: '500 نقطة' },
                        { offer: 'ترقية VIP مجانية', pts: '1000 نقطة' },
                        { offer: 'تجربة مجانية', pts: '2000 نقطة' }].map(o => (
                        <div key={o.offer} className="flex items-center justify-between p-2 rounded-lg bg-theme-card border border-theme-border">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            className="bg-theme-card rounded-2xl border border-theme-border p-4 flex items-center justify-center gap-6">
