      ...
      >
        <div className="rounded-2xl border border-theme-gold/20 bg-theme-surface p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold font-playfair text-white">إنشاء حساب جديد</h1>
            <p className="text-white/60 mt-1 text-sm font-cairo">انضم إلى EgyptHub اليوم</p>
          </div>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs text-white/40 font-cairo">أو بالبريد الإلكتروني</span>
            <div className="flex-1 h-px bg-white/10" />
          </div>

          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-sm font-medium text-white mb-1.5 font-cairo">الاسم الكامل</label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full bg-white/[0.04] border rounded-xl pr-10 pl-4 py-3 outline-none transition-all duration-200 focus:border-theme-gold/40 text-white placeholder-white/40 font-cairo ${
                    errors.name ? 'border-red-500' : 'border-theme-gold/[0.08]'
                  }`}
                />
              </div>
              <AnimatePresence>
                {errors.name && (
                  <motion.p initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    className="text-red-400 text-xs mt-1 font-cairo">{errors.name}</motion.p>
                )}
              </AnimatePresence>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-1.5 font-cairo">تأكيد كلمة المرور</label>
              <div className="relative">
                <svg className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                </svg>
                <input
                  ...
                  placeholder="••••••••"
                  className={`w-full bg-white/[0.04] border rounded-xl pr-10 pl-10 py-3 outline-none transition-all duration-200 focus:border-theme-gold/40 text-white placeholder-white/40 font-cairo ${
                    errors.confirmPassword ? 'border-red-500' : 'border-theme-gold/[0.08]'
                  }`}
              ...
              <AnimatePresence>
                {errors.confirmPassword && (
                  <motion.p initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    className="text-red-400 text-xs mt-1 font-cairo">{errors.confirmPassword}</motion.p>
                )}
              </AnimatePresence>
            </div>
          ...
          <p className="text-center text-sm text-white/60 mt-6 font-cairo">
            لديك حساب؟{' '}
            <Link href="/auth/social-login" className="text-theme-gold font-semibold hover:text-theme-gold/80 transition-colors">
              سجّل دخولك
            </Link>
          </p>
        </div>
