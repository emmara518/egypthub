...
export default function AdminNetworkPage() {
  ...
  return (
    <div className="min-h-screen bg-theme-bg text-white p-6">
      {/* Header */}
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-2xl font-bold mb-1">Network Management</h1>
        <p className="text-sm text-theme-secondary">Read-only overview of your network ecosystem</p>
      </motion.div>
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-8">
        {[
          ...
          <motion.div key={i} variants={item}
            className="bg-theme-card rounded-2xl border border-theme-border p-4 flex flex-col items-center text-center">
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${stat.color} flex items-center justify-center mb-3`}>
              <stat.icon className="text-white text-lg" />
            </div>
            <p className="text-xl font-bold font-english">{stat.value}</p>
            <p className="text-[10px] text-theme-secondary mt-0.5">{stat.label}</p>
          </motion.div>
        ))}
      </motion.div>

      {/* Tab Navigation */}
      <div className="flex gap-1 mb-6 bg-theme-surface rounded-xl p-1 border border-theme-border w-fit">
        {tabs.map((tab) => (
          ...
          <div className="bg-theme-card rounded-2xl border border-theme-border overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-theme-border text-theme-secondary text-[11px] uppercase tracking-wider">
                ...
                <tbody>
                  {partners.map((p, i) => (
                    <motion.tr key={p.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }}
                      className="border-b border-theme-border/50 hover:bg-theme-elevated/50 transition-colors">
                      <td className="px-4 py-3 font-english text-theme-secondary text-xs">{p.id}</td>
                      <td className="px-4 py-3 font-medium">{p.name}</td>
                      <td className="px-4 py-3 text-theme-secondary">{p.category}</td>
                ...
                <tbody>
                  {ambassadors.map((a, i) => (
                    <motion.tr key={a.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }}
                      className="border-b border-theme-border/50 hover:bg-theme-elevated/50 transition-colors">
                      <td className="px-4 py-3 font-english text-theme-secondary text-xs">{a.id}</td>
                      <td className="px-4 py-3 font-medium">{a.name}</td>
                      <td className="px-4 py-3">{a.city}</td>
                      <td className="px-4 py-3 text-theme-secondary">{a.role}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 text-xs">
                          <HiStar className="text-theme-gold text-xs" />
                ...
                <tbody>
                  {leads.map((l, i) => (
                    <motion.tr key={l.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }}
                      className="border-b border-theme-border/50 hover:bg-theme-elevated/50 transition-colors">
                      <td className="px-4 py-3 font-english text-theme-secondary text-xs">{l.id}</td>
                      ...
                      <td className="px-4 py-3 text-theme-secondary">{l.source}</td>
                ...
                <tbody>
                  {([
                    ...
                    <motion.tr key={r.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }}
                      className="border-b border-theme-border/50 hover:bg-theme-elevated/50 transition-colors">
                      <td className="px-4 py-3 font-english text-theme-secondary text-xs">{r.id}</td>
                      ...
                      <td className="px-4 py-3 text-theme-secondary">{r.source}</td>
                      <td className="px-4 py-3 font-english text-xs text-theme-secondary">
                ...
                <tbody>
                  {Array.from({ length: Math.max(commissionStats.total, 1) }, (_, i) => ({
                    ...
                    <motion.tr key={c.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }}
                      className="border-b border-theme-border/50 hover:bg-theme-elevated/50 transition-colors">
                      <td className="px-4 py-3 font-english text-theme-secondary text-xs">{c.id}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 text-xs">
