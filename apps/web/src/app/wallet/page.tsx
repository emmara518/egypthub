            ...
            <AnimatePresence mode="wait">
              {activeTab === 'balance' && (
                <motion.div key="balance" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                  ...
                  {/* Balance Card */}
                  <div className="rounded-2xl border border-theme-gold/20 bg-gradient-to-l from-theme-gold/10 to-theme-gold/5 p-6 mb-6 relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-32 h-32 bg-theme-gold/5 rounded-full -translate-x-1/2 -translate-y-1/2" />
                    <div className="relative">
                      <p className="text-xs text-white/40 font-cairo mb-1">الرصيد الحالي</p>
                      <p className="text-4xl font-bold text-theme-gold font-english mb-1">EGP {walletBalance.toLocaleString()}</p>
                      <p className="text-xs text-white/30 font-cairo">صالح للاستخدام</p>
                      <div className="flex gap-3 mt-5">
                        <button className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-theme-gold to-theme-gold text-theme-bg text-xs font-bold font-cairo hover:shadow-[0_4px_15px_rgba(212,162,76,0.3)] transition-all">
                          إيداع
                        </button>
                        <button className="px-5 py-2.5 rounded-xl border border-theme-gold/30 text-theme-gold text-xs font-bold font-cairo hover:bg-theme-gold/5 transition-all">
                          سحب
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div className="rounded-2xl border border-white/[0.06] bg-theme-surface p-5">
                      <p className="text-xs text-white/30 font-cairo mb-3">نقاط الولاء</p>
                      <div className="flex items-center justify-between">
                        <p className="text-xl font-bold text-theme-gold font-english">{loyaltyPoints.toLocaleString()} نقطة</p>
                        <div className="w-12 h-12 rounded-xl bg-theme-gold/10 flex items-center justify-center">
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="var(--gold)"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2z"/></svg>
                        </div>
                      </div>
                      <p className="text-[10px] text-white/30 font-cairo mt-2">المستوى: <span className="text-theme-gold font-medium">الذهب</span></p>
                    </div>
                    <div className="rounded-2xl border border-white/[0.06] bg-theme-surface p-5">
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        <div className="flex-1 h-1.5 rounded-full bg-theme-card">
                  ...
                  {/* Recent Transactions */}
                  <div className="rounded-2xl border border-white/[0.06] bg-theme-surface p-6">
                    <div className="flex items-center justify-between mb-4">
                      <p className="text-sm font-bold text-white mb-0 font-cairo">آخر المعاملات</p>
                      <button onClick={() => setActiveTab('transactions')} className="text-theme-gold text-xs font-cairo hover:underline">عرض الكل</button>
                    </div>
                    <div className="space-y-3">
                      {transactions.slice(0, 3).map((t, i) => (
                        <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-theme-card/50 border border-white/[0.04]">
                  ...
                  <div className="space-y-3">
                    {transactions.map((t, i) => (
                      <motion.div key={t.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                        className="flex items-center justify-between p-4 rounded-xl bg-theme-surface border border-white/[0.06] hover:border-theme-gold/15 transition-all">
              ...
              {activeTab === 'rewards' && (
                <motion.div key="rewards" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="bg-theme-gold/5 rounded-xl p-4 border border-theme-gold/10">
                      ...
                      <div className="w-full bg-theme-card rounded-full h-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {rewards.map((r, i) => (
                      <motion.div key={r.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                        className="rounded-2xl border border-white/[0.06] bg-theme-surface p-4 hover:border-theme-gold/20 transition-all">
                        <div className="flex items-center gap-4">
                ...
                <motion.div key="offers" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      <motion.div key={i} className="flex items-center gap-4 p-4 rounded-xl bg-theme-surface border border-white/[0.06] hover:border-theme-gold/15 transition-all cursor-pointer">
                        <div className="w-12 h-12 rounded-xl bg-theme-gold/10 flex items-center justify-center text-2xl shrink-0">
                          {offer.icon}
                        </div>
                        <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" strokeWidth="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>
