    ...
    <div className="min-h-screen bg-theme-bg">
      <div className="max-w-[1200px] mx-auto px-4 lg:px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-theme-muted font-cairo mt-1">تتبع جميع حجوزاتك ورحلاتك</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {filtered.length === 0 ? (
            <motion.div key="empty" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              ...
              <p className="text-sm text-theme-muted font-cairo mb-6">لم تقم بأي حجوزات بعد. ابدأ رحلتك الآن!</p>
              <Link href="/experiences"
                className="inline-flex px-6 py-3 rounded-xl bg-gradient-to-l from-theme-gold to-theme-gold text-theme-bg text-sm font-cairo font-bold">
                استكشف التجارب
              </Link>
            </motion.div>
          ) : (
                      ...
                      <div className="flex-1 p-4 sm:p-5">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-[10px] font-bold text-theme-gold font-english">{booking.id}</span>
                          ...
                          <span className="text-lg font-bold text-theme-gold font-english shrink-0">EGP {booking.price.toLocaleString()}</span>
                        </div>
                        <div className="flex flex-wrap gap-4 text-xs text-theme-secondary font-cairo mb-3">
                        ...
                        <div className="flex gap-2">
                          <Link href={`/booking/details`}
                            className="px-4 py-2 rounded-lg bg-theme-gold/10 text-theme-gold text-xs font-cairo font-medium hover:bg-theme-gold/20 transition-all">
                            عرض التفاصيل
                          </Link>
                          <button onClick={() => setExpandedId(isExpanded ? null : booking.id)}
                            className="px-4 py-2 rounded-lg border border-theme-border text-theme-secondary text-xs font-cairo hover:text-white hover:border-theme-gold/20 transition-all">
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                          ...
                          <div className="p-5 grid grid-cols-2 sm:grid-cols-4 gap-3">
                            {[
                              ...
                              <div key={item.label} className="bg-theme-surface rounded-xl p-3 text-center">
        ...
        <div className="mt-8 text-center">
          <p className="text-xs text-theme-muted font-cairo">إجمالي الحجوزات: <span className="text-theme-gold font-english font-bold">{filtered.length}</span></p>
        </div>
      </div>
    </div>
  );
